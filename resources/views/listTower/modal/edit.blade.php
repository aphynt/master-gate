<div id="editlistTower{{ $item->UUID }}" class="modal-demo">
    <div class="d-flex w-100 p-3 bg-primary align-items-center justify-content-between">
        <h4 class="custom-modal-title">Edit Tower</h4>
    ...
    <div class="custom-modal-text text-muted">
        <form action="{{ route('listTower.update', $item->UUID) }}" method="GET">
            @csrf
            <div class="mt-3">
                <div class="form-group">
                    <label for="variableForm">Variable</label>
                    <input type="text" class="form-control" id="variableForm" value="{{ $item->NAMA }}" name="NAMA" required>
                </div>
            </div>
            <div class="mt-3">
