...
class SummaryMonthlyAdditionalExport implements FromCollection, WithEvents, WithStyles, WithDrawings, Responsable, WithTitle
{
    use \Maatwebsite\Excel\Concerns\Exportable;

    private $additionalMonthlyActivity;
    private $nonMtStart;
    private $fileName = 'Additional_Monthly_FMS_Report.xlsx';

    public function __construct(Collection $additionalMonthlyActivity, $nonMtStart)
    {
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // Table headers
                $startRow = 3;
                $headers = ['No', 'Tanggal', 'Team', 'Aktivitas', 'On-site', 'Reporting'];
                $col = 'A';
                ...
                $sheet->getStyle("A{$startRow}:F{$startRow}")->getFill()->setFillType('solid')->getStartColor()->setRGB('D9E1F2');
                ...
                foreach ($this->additionalMonthlyActivity as $index => $item) {
                    ...
                    $sheet->setCellValue("B{$row}", $item->DATE_REPORT
                        ...
                        : '');
                    $sheet->setCellValue("C{$row}", $item->NAMA_TEAM);
                    $sheet->setCellValue("D{$row}", $item->ACTION_PROBLEM);
                    $sheet->setCellValue("E{$row}", $item->PIC);
                    $sheet->setCellValue("F{$row}", $item->REPORTING);
                ...
                // Border
                $sheet->getStyle("A{$startRow}:F" . ($row - 1))->getBorders()->getAllBorders()
