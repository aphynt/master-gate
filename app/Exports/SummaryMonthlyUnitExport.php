...
class SummaryMonthlyUnitExport implements FromCollection, WithEvents, WithStyles, WithDrawings, Responsable, WithTitle
{
    use \Maatwebsite\Excel\Concerns\Exportable;

    private $unitMonthlyActivity;
    private $nonMtStart;
    private $fileName = 'Unit_Monthly_FMS_Report.xlsx';

    public function __construct(Collection $unitMonthlyActivity, $nonMtStart)
    {
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A1', 'Laporan Aktivitas Unit FMS (Hardware Focus) Bulan '. \Carbon\Carbon::parse($this->nonMtStart)->translatedFormat('F Y') );
                $sheet->getStyle('A1')->getFont()->setSize(14)->setBold(true);
                $sheet->getStyle('A1')->getAlignment()->setHorizontal('center');

                // Table headers
                $startRow = 3;
                $headers = ['No', 'Tanggal', 'No Unit', 'Deskripsi Problem', 'Action Problem', 'Remarks', 'On-site', 'Reporting'];
                $col = 'A';
                ...
                foreach ($this->unitMonthlyActivity as $index => $item) {
